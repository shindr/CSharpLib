...
namespace Shindr.CSharpLib
{
	...
	/// </summary>
	public interface IViewModelWrapper<T> : INotifyPropertyChanged
	{
		/// <summary>
		...
		where TViewModel : class, IViewModelWrapper<TModel>, new()
	{
		...
		{
			RecycleViewModels = !typeof(TModel).IsValueType;
		}
		...
		TViewModel ViewModelFactory(TModel model)
		{
			...
			if (result == null)
			{
				...
				result.PropertyChanged += ViewModelPropertyChanged;
		...
		{
			if("Model".Equals(e.PropertyName))
			{
				var viewModel = (TViewModel)sender;
				int idx = IndexOf(viewModel);
				if (idx > 0 && idx < Model.Count)
					Model[idx] = viewModel.Model;
			}
		}
